 ...
 *
 */
 //cấp phát thêm vm region (vm_rg) tại break point của vm_area theo size
...
{
  //lấy list vm_area để kiểm tra sự trùng nhau giữa vm_area
  struct vm_area_struct* vma = caller->mm->mmap;

  /* TODO validate the planned memory area is not overlapped */
  //xác định vm_area theo id để kiểm tra vm_area này có trùng với vm_area khác không
  // struct vm_area_struct* cur_vma = get_vma_by_num(caller->mm, vmaid);
  while(vma) {
 ...
 */
/*
Hàm find_victim_page được thiết kế để tìm một trang thích hợp để thay thế (victim page) trong bộ nhớ ảo của tiến trình,
 Mô tả
 Tìm trang đầu tiên có PTE
 * - Duyệt qua danh sách các trang nhớ được quản lý bởi cấu trúc FIFO.
 * - Tìm trang đầu tiên có PTE 
 * - Đánh dấu trang này là có thể thay thế và trả về số trang của nó.
*/
